...
def find_ticket_count(order_string):
    if order_string.find("2 ×") != -1:
        return 2
    elif order_string.find("3 ×") != -1:
        return 3
    elif order_string.find("4 ×") != -1:
        return 4
    elif order_string.find("5 ×") != -1:
        return 5
    elif order_string.find("6 ×") != -1:
        return 6    
    else:
        return 1
    
df = pd.DataFrame(columns=["datetime", "SEK", "Single tickets bought", "Day", "Month", "Year", "weekday"])
...
with iml.IMAP4_SSL("imap.yourhost.domain", 993) as M:
    ...
    M.login("yourusername@yourhost.domain", "yourPassword")
    # select correct folder - return values indicate if it exists and how many emails are to be processed
                    ...
                    for part in msg.walk():
                        ...
                        if content_type == "text/html":
                            ...
                            if string_list[betalt_index+1].find("kr") != -1:
                                money_spent = string_list[betalt_index+1].strip(" ").split("(")[0][0:-3]
                            if string_list[betalt_index+1].find("SEK") == 0:
                                money_spent = string_list[betalt_index+1].strip(" ").split("(")[0][3:]
                            elif string_list[betalt_index+1].find("SEK") != -1:
                                money_spent = string_list[betalt_index+1].split("SEK")[0].strip(" ").replace("\xa0", "")
                            ...
                            df.loc[len(df)] = {"datetime": msg_date[5:-6], "SEK": float(money_spent.replace(",",".")), "Single tickets bought":ticket_count}
                else:
